...
**Restoration & CD transfer** — an analysis-first vinyl workbench detects clicks, pops and flat-topped clipping while protecting musical attacks; includes adjustable click repair, peak reconstruction/declipping, learned or automatic noise profiling, hiss reduction and 50/60 Hz hum removal. Gentle/Balanced/Strong presets, wet/dry control, bypass A/B and bounded live previews leave the source untouched until one undoable apply. The handoff auto-splits quiet gaps into reorderable, splittable tracks with editable In/Out points, previews the exact dry or rack-rendered path, then emits gapless, 588-frame-aligned 44.1 kHz/16-bit WAV files plus a CUE sheet through staged, cancellation-safe publishing.
...
**Recording** — WASAPI capture with device picker and live meters; a transport **Arm** control starts the selected persisted input immediately when Record is pressed; metronome count-in at any BPM; click track while recording; punch-insert into the current selection with smoothed joins. Capture finalization waits for the device tail, keeps interrupted takes recoverable, and marks every new recording as unsaved.
