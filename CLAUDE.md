...
- `Audio/DsdCodec` imports uncompressed DSD64/DSD128 DSF and DSDIFF as a new 44.1 kHz / 32-bit-float working copy; native DSD is never made eligible for in-place Save. DSF is channel-blocked and may be LSB-first (`BitsPerSample = 1`) or MSB-first (`8`); DSDIFF is byte-interleaved and MSB-first. Conversion is a streaming two-stage decimator: a 128-bit FIR prevents the initial 8:1 fold, then a long Blackman-Harris FIR runs through `PartitionedConvolver` before the final integer decimation. The final filter's −6 dB midpoint is 21 kHz, not 20 kHz, so 20 kHz remains flat while the response is stopped below the 22.05 kHz Nyquist limit. Output carries a 2× reference gain because 0 dB-SACD is 50% of the theoretical 1-bit modulation range and AudioGate maps that reference to 0 dBFS; float retains the legal headroom above it. DST-compressed DSDIFF is rejected explicitly. The 1.5 GB decoded ceiling is the same practical memory class as capture finalization and keeps a complete LP side viable without letting a multi-hour DSD file allocate the machine.
...
- Playback: NAudio 3 `WasapiPlayer` shared/exclusive mode ← `MasterSection` (StudioEq → Limiter → meters/LUFS/FFT ring) ← `DocumentProvider`.
