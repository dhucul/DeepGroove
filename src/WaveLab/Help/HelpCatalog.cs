...
public static class HelpCatalog
{
    ...
    [
        ...
        Topic("files", "FILES", "Files, tabs, Save, and Export",
            ...
            "open save save as export recent drag drop bit depth wav mp3 flac m4a wma tabs",
            Section("Open and Open As Bit Depth",
                "Open accepts supported WAV and media formats; availability of compressed formats depends on Windows codecs. Open As Bit Depth creates a converted working copy as 16-bit PCM with or without dither, 24-bit PCM, or 32-bit float. Conversion does not improve the quality of a lower-resolution source."),
            Section("Save versus Save As",
                "Save writes the current WAV document back to its existing path. A new recording or generated document has no path, so Save asks for one. Save As chooses a new WAV path and encoding. Saving marks that document clean; it does not close the tab."),
            Section("Export",
                "Export creates a separate delivery file without changing the open document. Choose 32-bit float WAV, 24-bit WAV, dithered or undithered 16-bit WAV, FLAC when available, MP3, AAC/M4A, or WMA. Lossy formats expose a bitrate. You can keep the current sample rate or convert to 44.1, 48, 88.2, or 96 kHz, and export the whole file or the current selection."),
            ...
            Section("Bit-depth guidance",
                "Use 32-bit float for an intermediate master that may be processed again, 24-bit PCM for high-resolution delivery, and dithered 16-bit PCM for CD-compatible output. MP3, AAC, and WMA are smaller lossy listening copies. FLAC is lossless.")),
        ...
        Topic("transport", "PLAYBACK", "Transport, playback, loop, and Arm",
            ...
            Section("Playback controls",
                "Play/Stop or Space starts playback at the current position and stops it. Stop always stops the active transport. Home returns to the beginning. Loop repeats the selection when a selection exists; otherwise it repeats the playable document range."),
        ...
        Topic("recording-levels", "RECORDING", "Recording and the Level Assistant",
            ...
            "record vinyl input device mix format check levels true peak projected rms crest clipping gain count-in punch",
            ...
            Section("Assistant readouts",
                "True Peak estimates intersample peaks in dBTP. Projected adds a conservative reserve for louder peaks not yet heard. Program RMS is average active-program level in dBFS. Crest is the difference between peak and average level; a larger value means sharper transients. L/R reports persistent channel imbalance. Clipping reports digital rail hits, invalid device data, or repeated flat tops that may indicate upstream clipping."),
        ...
        Topic("settings", "SETTINGS", "Settings, autosave, and recovery",
            ...
            "settings general audio input output buffer autosave recovery export default undo",
            Section("General",
            ...
            Section("Audio",
                "Output Device chooses playback. Default Input Device is used by Arm and preselected in Record Setup. Playback Buffer trades latency for stability: smaller is more responsive, larger is safer on a busy system, and it applies to the next playback."),
            Section("Autosave",
        ...
        Topic("shortcuts", "REFERENCE", "Keyboard and mouse shortcuts",
            ...
            Section("Transport",
                "Space - Play/Stop. Home - Go to Start. Ctrl+R - Record or Stop Recording. Alt+Left - Previous Marker. Alt+Right - Next Marker."),
            Section("Editing",
