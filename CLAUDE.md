...
- The Recent Files submenu is a `CompositeCollection`, because a `MenuItem` cannot carry both an `ItemsSource` and a fixed child and Clear Recent Files is a fixed child. A `CollectionContainer` sits in neither the visual nor the logical tree, so the bound half reaches the view model through the `BindingProxy` resource `ShellContext` rather than through `DataContext`: a plain `{Binding}` written there resolves against nothing and **fails silently** — the submenu still opens, still shows Clear, and simply lists no files. Everything declared inside that collection carries its own `Style`, which is what keeps the item-container style (turn a string into an Open command) from being applied to it. `RecentFilesMenuTests` counts the paths rather than looking only for the new entry, for that reason.
