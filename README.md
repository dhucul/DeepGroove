...
**Recording** — WASAPI capture with device picker and live peak/RMS/hold meters; a vinyl-oriented **Recording Level Assistant** runs a discard-safe preflight scan, measuring intersample true peak, a click-resistant programme peak (isolated pops—even rail-touching ones—can't set the gain for a whole side), projected peak with a crest-aware unseen-transient reserve, program RMS and integrated LUFS, noise floor, channel balance, DC offset, 50/60 Hz hum and sustained clipping — then recommends a linked input-gain reduction or an optional conservative increase before the take. A scrolling 30-second input-history strip shows the scan against the target band, settled outcomes are remembered per input device, and the check summary travels into the take's metadata. The monitor stream switches atomically into retained recording so calibration audio never enters the take. An automatic needle-drop mode listens while armed, starts on the stylus-contact click, and preserves a 250 ms safety pre-roll so the opening groove is intact. A transport **Arm** control starts the selected persisted input immediately when Record is pressed; metronome count-in at any BPM; click track while recording; punch-insert into the current selection with smoothed joins. Capture finalization waits for the device tail, keeps interrupted takes recoverable, and marks every new recording as unsaved.
