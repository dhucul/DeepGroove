        ...
        lock (_sync)
        {
            ...
            for (int index = offset; index < end; index += _channels)
            {
                ...
                for (int channel = 0; channel < _channels; channel++)
                {
                    ...
                    if (magnitude > 1e-9)
                    {
                        // A logarithm, deliberately. A binary search over precomputed bin
                        // edges avoids it and measures 8.5x slower: Math.Log10 is a
                        // hardware-assisted intrinsic and seven unpredictable branches are
                        // not. 2.98 ms against 25.52 for ten seconds of 48 kHz stereo,
                        // which is 0.03% of realtime either way - this was never the cost
                        // on this thread that counting the calls made it look like.
