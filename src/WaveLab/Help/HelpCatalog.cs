    ...
    [
        ...
        Topic("recording-levels", "RECORDING", "Recording and the Level Assistant",
            ...
            Section("Device Input Level",
                "The Device Input Level slider controls the selected Windows capture endpoint in dB when its driver exposes that control. Its step size comes from the device driver and may be 1.5 or 2 dB. WaveLab Fine Trim provides attenuation from 0 to -3 dB in 0.1 dB steps to fill the gap between device settings. Fine Trim is applied to the meters and retained recording, but it cannot repair clipping that already occurred in an analogue preamp or converter; reduce gain at that hardware when the assistant reports input or upstream clipping. Changing either control automatically clears the old level-check measurements."),
            ...
            Section("Safe setting for the side",
                "Safe Setting for This Side converts the recommendation into an exact Device plus Fine Trim setting. Once the scan settles, WaveLab holds the most conservative recommendation encountered, so the number does not rise again when a quieter passage follows. Scan the loudest passage at minimum; playing the whole side through Check Levels gives the safest whole-record setting. A downward change can still occur if a later passage is louder. An optional increase is deliberately conservative: only use it when the analogue chain has known headroom and noise is a real problem. With 24-bit or 32-bit-float capture, a somewhat low clean recording is normally safer than clipping and can be normalized later. Never adjust left and right independently from a short musical passage."),
