    ...
    [
        ...
        Topic("restoration", "RESTORATION", "Vinyl restoration and cleanup",
            ...
            Section("Vinyl Restoration & CD Transfer",
                "Choose the source signal before trusting the analysis. RIAA already applied keeps the ordinary restoration path. Flat cartridge transfer removes DC, measures azimuth and wow, repairs clipping/clicks/crackle before the selected playback curve, then runs rumble, vertical-noise, hum and hiss cleanup after it. Continuous-noise analysis is performed on a temporary playback-equalised copy, while impulse analysis stays on the raw transfer. The source is untouched until Apply; Apply & Prepare CD continues directly into track preparation. Editing the recording makes the analysis stale and withholds Apply until Re-analyze scans it again."),
            Section("Flat transfer stages",
                "Restore > Flat Vinyl Transfer Workflow opens directly in flat mode. RIAA (1954) and minimum phase are the defaults for a modern LP or 45; historical discs can choose another curve. DC removal starts enabled. Azimuth is offered only for a measurable stereo delay. Wow and flutter is measured for every transfer but remains measure-only unless high coverage and a substantial deviation support resampling. A flat source cannot be mixed with its equalised output, so Output Mix is fixed at 100% and Keep what was removed is unavailable in that mode."),
